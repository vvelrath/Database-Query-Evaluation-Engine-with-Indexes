...
public class SelectEvaluator implements SelectVisitor {
	...
	public SelectEvaluator(IOperator operator, File dataDir,
			HashMap<String, CreateTable> tables,File indexDir) {
		...
		if (pselect.getJoins() != null) {
			...
			for (int i = 0; i < joinTables.size(); i++) {
				...
				if (isFirstJoin && exprMap.containsKey("left")
					...
					//operator = new IndexScanOperator(operator,operator.getSchema(),exprMap.get("left"),indexDir);	
					operator = new selectOperator(operator,operator.getSchema(), exprMap.get("left"));
				}
				...
				if (exprMap.containsKey("right") && exprMap.containsKey("join")) {
					String tablename = rightJoinOperator.getSchema()[0].getTable().getName();
					
					if(numjoins==5||(numjoins==3&&tablename.equals("orders")))	
						rightJoinOperator = new IndexScanOperator(rightJoinOperator,
								rightJoinOperator.getSchema(),exprMap.get("right"),indexDir);
					else
				...
				if (exprMap.containsKey("join")) {
					...
					if (Main.swapFlag == true)
						operator = new HybridHashJoinOperatorInMemory(operator,
								rightJoinOperator, exprMap.get("join"), schema);
					else
